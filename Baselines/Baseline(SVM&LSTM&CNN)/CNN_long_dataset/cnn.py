...
from customCallbacks import MyEarlyStopping, MyModelCheckpoint
...
class ConvNN(object):
	...
	def setup(self,X_train_shape):
		...
		conv1 = Conv3D(16,kernel_size=(X_train_shape[2], 5, 5),padding='valid',strides=(1,2,2),name='conv1')(normal1)
		...
		ts = int(np.round(np.ceil((X_train_shape[3]-4)/float(2)) / 2 + 0.1))
		fs = int(np.round(np.ceil((X_train_shape[4]-4)/float(2)) / 2 + 0.1))
		...
		pool2 = MaxPooling2D(pool_size=(2,2),padding='same')(relu2)
